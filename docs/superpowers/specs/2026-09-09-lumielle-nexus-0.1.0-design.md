...
启动时向群发送标题、字段和填写说明，可选 `@全体`。解析器支持中英文冒号、字段乱序和单字段纯文本；缺少字段仍保存 partial entry 并回复缺失字段（可配置关闭确认）。状态查询返回提交数、尽力获取的群成员数、未提交数和状态。停止后生成三个 sheet：`统计结果`、`未提交成员`、`任务信息`，并尽力通过 OneBot `upload_private_file` 回传创建者；回传失败不回滚导出或任务结果。
