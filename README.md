...
支持中文或英文冒号、字段乱序和单字段纯文本。停止任务后，会在 plugin data 的 `exports/` 生成包含 `统计结果`、`未提交成员`、`任务信息` 三个 sheet 的 `.xlsx`。如果 OneBot 无法取得完整群成员名单，导出仍会保留，但未提交人数无法准确计算。QQ 文件回传失败也不会丢失导出文件。
